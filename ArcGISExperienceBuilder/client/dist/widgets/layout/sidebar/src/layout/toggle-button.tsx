...
export class SidebarController extends React.PureComponent<SidebarControllerProps> {
  ...
  render() {
    ...
    return (
      <Button className="sidebar-controller d-flex justify-content-center align-items-center"
        css={this.getStyle()} style={this.flipStyle(style)}>
        <Icon className="icon" rotate={expanded ? 0 + rotate : 180 + rotate}
          icon={ICONS[icon]} size={iconSize}></Icon>
      </Button>
    );
  }
}
